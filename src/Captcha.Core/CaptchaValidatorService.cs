...
namespace Captcha.Core
{
    ...
    {
        private readonly ILogger<CaptchaValidatorService> _logger;
        ...
        public CaptchaValidatorService(
            ...
            ILogger<CaptchaValidatorService> logger,
            ...
            DisplayMode captchaGeneratorDisplayMode)
        {
            ...
            {
                _logger.LogDebug($"Ignoring ValidateCaptcha during `{httpContext.Request.Method}`.");
                return true;
            }
        ...
        private (string captchaText, string inputText, string cookieToken) getFormValues(HttpContext httpContext)
        {
            ...
            if (!httpContext.Request.HasFormContentType)
            {
                throw new InvalidOperationException(
                    $"Captcha expects `{captchaHiddenInputName}`, `{captchaInputName}` & `{captchaHiddenTokenName}` fields to be present in the posted `form` data." +
