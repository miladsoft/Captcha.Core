    ...
    public class CaptchaImageController : Controller
    {
        ...
        public IActionResult Refresh(string data)
        {
            try
            {
                ...
                var model = _serializationProvider.Deserialize<CaptchaTagHelperHtmlAttributes>(decryptedModel);
                if (model == null)
                {
        ...
        private void invalidateToken(CaptchaTagHelperHtmlAttributes model)
        {
