...
namespace Captcha.Core
{
    ...
    public class CaptchaTagHelper : CaptchaTagHelperHtmlAttributes, ITagHelper
    {
        ...
        private TagBuilder getCaptchaImageTagBuilder(ViewContext viewContext, string encryptedText)
        {
            ...
            _urlHelper.Action(
                            action: nameof(CaptchaImageController.Show),
            ...
            var CaptchaImg = $"{_captchaOptions.CaptchaClass}Img";
            captchaImage.TagRenderMode = TagRenderMode.SelfClosing;
            captchaImage.Attributes.Add("id", CaptchaImg);
            captchaImage.Attributes.Add("name", CaptchaImg);
        ...
        private TagBuilder getRefreshButtonTagBuilder(ViewContext viewContext, string captchaDivId, string captchaToken)
        {
            ...
            var actionUrl = UseRelativeUrls ?
            _urlHelper.Action(
                action: nameof(CaptchaImageController.Refresh),
                controller: nameof(CaptchaImageController).Replace("Controller", string.Empty, StringComparison.Ordinal),
                values: new { data = encryptSerializedValues, area = "" }) :
            _urlHelper.Action(
                action: nameof(CaptchaImageController.Refresh),
            ...
            var CaptchaRefreshButton = $"{_captchaOptions.CaptchaClass}RefreshButton";
            refreshButton.Attributes.Add("id", CaptchaRefreshButton);
            refreshButton.Attributes.Add("name", CaptchaRefreshButton);
